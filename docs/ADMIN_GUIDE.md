...
| 메일 알림 | 사내 SMTP 릴레이(`mail.smtp_host` …), 보내는 사람, 이벤트별 스위치, 시험 발송, 발송 기록 — 기본은 꺼짐 | 부록 12 |
...
- **AI 로 나가는 것** — 생각 본문은 사용자가 `Dream 분석에서 제외`하거나 공간에서 `AI Dream 분석`을 끄면 어디로도
  ...
  같은 사실을 보여 줍니다.
- **메일로 나가는 것** — 기본 꺼짐. 켜면 다섯 가지 이벤트(검토 요청·결과, 공간 공유, 댓글 언급, 내 생각의 댓글)가
  관리자가 적은 **사내 릴레이**로만 나갑니다(부록 12). 릴레이 비밀번호는 암호화되어 저장되고 설정 API 는 `설정됨`만
  돌려줍니다. 발송 기록에는 제목과 받는 사람만 있고 본문은 없습니다.
...
## 부록 12. 메일 알림 (`/admin/mail`)

누군가 기다리고 있습니다 — 자기에게 온 줄도 모르는 검토 요청을 기다리는 팀장, 결과를 기다리며 검토 화면을 새로고침하는
요청자, 공유된 줄 모르는 공간, 댓글에 불린 이름. 지금까지는 umm 을 열어 봐야만 알 수 있었습니다. 이 화면은 사내 표준
(`MAIL-STANDARD.md`)대로 **사내 SMTP 릴레이**를 적어 그 다섯 가지를 메일로 보내게 합니다. 기본은 **꺼짐**이고, 새로
설치한 곳에서는 아무것도 달라지지 않습니다.

### 설정

설정 키는 `mail` 이며 `app_settings` 에 저장됩니다. 표준의 키 이름에서 `mail.` 을 뗀 것이 각 항목입니다.

| 키 | 기본값 | 뜻 |
| :--- | :--- | :--- |
| `mail.enabled` | `false` | 꺼짐이 기본. 켜려면 릴레이 주소가 있어야 저장됩니다 |
| `mail.smtp_host` | — | 사내 릴레이의 호스트 이름 또는 IP (포트는 따로). 폐쇄망이면 `postra` 를 가리키는 조합을 권합니다 |
| `mail.smtp_port` | `25` | 사내 릴레이는 대개 25 |
| `mail.security` | `auto` | `auto` · `none` · `starttls` · `tls`. `auto` 는 릴레이가 STARTTLS 를 알리면 쓰고 아니면 평문. 465 이면 `tls` 로 봅니다 |
| `mail.skip_tls_verify` | `false` | 사내 사설 인증서일 때만 |
| `mail.username` · `mail.password` | 빈 값 | **선택.** 인증 없는 릴레이가 흔합니다. 비밀번호는 다른 비밀과 같이 AES-256-GCM 으로 암호화되어 저장되고, 설정 API 는 `••••••••` 와 `password_configured: true` 만 돌려주며, 그 마스크를 그대로 저장하면 저장된 값이 유지됩니다. 로그에도 남지 않습니다 |
| `mail.from_address` · `mail.from_name` | — · `umm` | 보내는 사람. 주소를 비우면 `umm@<릴레이 주소>` 를 씁니다 — 릴레이가 발신 도메인을 검사하면 실제 주소를 적습니다 |
| `mail.base_url` | — | 메일 속 "바로 열기" 링크의 앞부분. 비우면 **일반 → 공개 URL** 을 씁니다 |
| `mail.timeout_seconds` | `10` | 연결부터 QUIT 까지 한 통에 허용하는 시간 (1~120) |
| `mail.notify_approval_request` | `true` | 검토 요청이 도착함 → 그 팀의 팀장과 모든 관리자 |
| `mail.notify_approval_decision` | `true` | 내 요청이 승인·반려됨 → 요청한 사람 |
| `mail.notify_space_shared` | `true` | 공간이 나에게 공유됨(직접 또는 승인으로) → 공유받은 사람 |
| `mail.notify_mention` | `true` | 댓글에서 나를 언급함 → 언급된 사람 |
| `mail.notify_comment` | `true` | 내 생각에 댓글이 달림 → 생각의 작성자 (언급된 경우는 언급 메일 한 통만) |

API 로는 `PUT /api/v1/admin/settings/mail` 에 위 항목을 (접두사 없이) 보냅니다. 켜 둔 채 `smtp_host` 를 비우면 `400` 입니다.

### 어떤 일을 보내는가, 그리고 왜 이것만인가

기준은 하나입니다 — **이 메일이 오지 않으면 누군가 손해를 보거나 화면을 계속 새로고침한다.** 위 다섯 가지가 그것이고,
"무언가 바뀜"(생각 수정, 연결 추가, 밤사이 Dream)은 아무도 기다리지 않으므로 보내지 않습니다.

시끄러우면 사람들은 규칙으로 통째로 버리고, 그러면 정말 필요한 알림도 함께 버려집니다. 그래서:

- **자기가 한 일은 자기에게 보내지 않습니다.** 관리자가 자기 요청을 승인해도, 자기 생각에 자기가 댓글을 달아도 메일은 없습니다.
- **한 번의 작업은 한 사람에게 한 통입니다.** 공유 승인은 요청자에게 결과 한 통, 공유받는 사람에게 공유 한 통이고, 댓글에서
  언급된 사람은 언급 메일만 받습니다(같은 댓글의 "내 생각에 댓글" 은 겹치지 않음). 검토자 목록에 같은 주소가 두 번
  있어도 한 통입니다.
- **받을 사람은 계정의 메일 주소로 정합니다.** 메일 기능은 자기 명부를 갖지 않고 `users.email` 만 읽습니다 — 주소가 없거나
  비활성인 계정에는 시도조차 기록하지 않습니다. Keycloak SSO 로 들어온 계정은 주소가 자동으로 채워집니다.
- **관리자는 이벤트 종류별로 끌 수 있습니다** (위 `notify_*`).

### 요청을 막지 않는다

메일은 **배경에서** 보냅니다. 댓글이나 승인은 평소처럼 답하고, 릴레이 연결은 그 뒤에 다른 고루틴에서 일어납니다. 릴레이가
느리거나 죽어 있어도 그 요청은 정상으로 끝나고, 실패는 발송 기록에만 남습니다. 한 통에 두 번까지 시도합니다(2초 간격) —
연결 하나를 거절했다가 다음 것을 받는 릴레이는 흔하기 때문입니다.

### 시험 발송

**메일 알림 → 시험 발송**: 받는 사람(비우면 내 계정 주소)을 적고 누르면 **저장된 설정**으로 실제 한 통을 보내고 릴레이의
답을 그 자리에서 보여 줍니다(`POST /api/v1/admin/mail/test`). 알림을 켜지 않아도 됩니다 — 릴레이를 먼저 증명하고 그다음
켜는 순서가 맞습니다. 저장하지 않은 변경이 있으면 단추가 잠기고 그 사실을 알려 줍니다.

릴레이의 말이 그대로 옵니다. 자주 보는 것:

| 답 | 뜻 |
| :--- | :--- |
| `SMTP 릴레이 … 에 연결하지 못함: … connection refused` | 주소·포트가 틀렸거나 방화벽. `docker compose exec umm nc -vz relay.intra 25` |
| `STARTTLS 실패: … certificate` | 사설 인증서. `skip_tls_verify` 를 켜거나 릴레이 인증서를 신뢰 저장소에 |
| `릴레이가 STARTTLS 를 제공하지 않습니다` | `security` 를 `auto` 나 `none` 으로 |
| `릴레이가 인증을 받지 않습니다` | 사용자 이름을 비웁니다 |
| `MAIL FROM 거절` / `RCPT TO 거절` | 릴레이가 발신 도메인이나 수신자를 제한. `from_address` 를 실제 주소로 |

### 발송 기록

같은 화면 아래 카드(`GET /api/v1/admin/mail/deliveries?status=&limit=`)에 **무엇이 건물 밖으로 나갔는지**가 있습니다 —
시도마다 한 줄, 언제·어떤 이벤트·누구에게·제목·성공/실패·시도 횟수·릴레이의 답. 실패만 남기면 "안 왔다" 는 문의에 답할 수
없으므로 성공도 남깁니다. **본문은 담지 않습니다** — 제목과 받는 사람이면 충분하고, 본문까지 담으면 이 표가 댓글의 사본이
됩니다. 90일 뒤에 지워집니다(`mail_deliveries`, 다음 기록을 남길 때 정리).

### 확인 절차

1. 새로 설치한 곳: 관리자 화면 **메일 알림**이 꺼져 있고 발송 기록이 비어 있습니다. 검토 요청을 만들어도 아무것도 나가지 않습니다.
2. 릴레이 주소만 적고 저장 → **시험 발송** → 받은 편지함에 `[umm] SMTP 시험 발송` 이 오는지, 발송 기록에 `test · 성공` 이
   남는지 봅니다. 실패하면 위 표대로 고칩니다.
3. 켜고 저장한 뒤, 검토 프로세스를 켠 상태에서 일반 사용자로 검토 요청을 만듭니다 → 팀장과 관리자에게 `[umm] 검토 요청: …` 이
   오고 요청자에게는 오지 않습니다. 팀장이 반려하면 요청자에게 `[umm] 검토 결과: …` 가 옵니다.
4. 릴레이를 끊어 놓고(주소를 틀리게) 같은 일을 하면 요청은 평소처럼 성공하고, 발송 기록에 `실패 · 2회` 와 연결 오류가 남습니다.
5. `GET /api/v1/admin/settings` 의 `mail.password` 가 `••••••••` 인지, `docker compose logs umm | grep -i password` 에
   아무것도 없는지 봅니다.
