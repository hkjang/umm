...
- **하이브리드 검색**: 키워드 일치와 로컬 의미 유사도를 결합하고 공간·종류·수정 시각 필터, 점수와 추천 이유를 제공합니다.
- **백링크·댓글·멘션**: 실제 연결선 방향을 확인하고 포스트잇 안에서 댓글을 남기며 `@username`으로 공간 멤버를 알립니다.
- **오프라인 큐**: 연결이 끊기면 변경을 장치에 보관하고 다시 연결될 때 멱등 키로 재전송합니다.
...
- 사설·예약 IP SSRF 차단, 세 번 재시도, 연속 10회 실패 시 자동 중지를 적용합니다.
