...
- **공간별 AI 제외**: 공간 관리에서 전체 공간의 AI Dream 분석을 즉시 켜거나 끔
...
- **오프라인 큐**: 연결이 끊기면 변경을 장치에 보관하고 다시 연결될 때 멱등 키로 재전송하며, 영구 거부된 요청은 사유를 알리고 제거합니다.
...
- 도메인 변경과 PostgreSQL outbox를 원자적으로 커밋해 재시작 뒤에도 이어 보내며, 사설·예약 IP SSRF 차단, 세 번 재시도, 연속 10회 실패 시 자동 중지를 적용합니다.
