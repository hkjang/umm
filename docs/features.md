...
- **백링크·댓글·멘션**: 실제 연결선 방향을 확인하고 포스트잇 안에서 댓글을 남기며 `@username`으로 공간 멤버를 알립니다. 댓글 작성 transaction은 현재 멤버십을 끝까지 잠그고, 작성자도 현재 공간 접근권한이 있을 때만 자신의 댓글을 삭제할 수 있습니다. 생각이 삭제되면 보관한 댓글 ID로 해결·재개·삭제 이벤트를 만들 수 없고 기존 댓글·멘션 알림도 목록과 unread count에서 숨깁니다.
