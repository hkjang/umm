...
4. 필요할 때만 AI Gateway를 저장합니다. 임베딩 모델을 비워 두면 외부 호출 없이 내장 로컬 임베딩을 사용합니다. 저장된 `enc:` API key를 현재 keyring으로 복호화하지 못하는 경우에도 ciphertext를 Gateway로 보내지 않고 로컬 임베딩으로 fail closed 하므로, 관리자 암호화 상태에서 `unreadable=0`을 확인한 뒤 원인을 복구하세요.
