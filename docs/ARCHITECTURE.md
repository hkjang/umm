...
- v0.8.0부터 관리자가 AI Gateway에 임베딩 모델을 지정하면 OpenAI 호환 `/v1/embeddings`를 대신 사용합니다. 모든 벡터에는 생성 알고리즘이 함께 저장되고 **같은 알고리즘끼리만 비교**하므로, 모델을 바꿔도 서로 다른 벡터 공간이 뒤섞이지 않고 점진적으로 다시 임베딩됩니다.
