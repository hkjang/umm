...
- `internal/intelligence/quality_test.go`가 라벨 데이터셋으로 활성 알고리즘의 판별력을 CI에서 보고하고, 나빠지면 실패합니다. 후보 모델은 `UMM_EMBEDDING_TEST_URL`/`UMM_EMBEDDING_TEST_MODEL`로 같은 기준에서 비교합니다.
