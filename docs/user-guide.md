...
### 🖼️ 생각에 그림 붙이기
- 메모 메뉴에서 **`그림 붙이기`** 를 누르고 파일을 고릅니다. 화이트보드 사진, 화면 캡처, 손으로 그린 그림 — 문장이 아닌 부분도 생각 옆에 둡니다.
- 카드 위에 바로 보입니다. 누르면 원본이 새 탭에서 열리고, 그림 오른쪽 위 `×` 로 뗍니다.
- **PNG · JPEG · GIF · WebP** 만 됩니다. 한 장 5MB, 생각 하나에 8장까지입니다. 넘으면 잘리지 않고 **얼마까지인지 알려 주며 거절**합니다.
- 확장자를 바꿔도 소용없습니다 — umm은 **파일 내용을 읽어서** 판단합니다. `SVG` 는 스크립트를 담을 수 있어 받지 않습니다.
- **생각을 지우면 붙인 그림도 함께 사라집니다.**
...
같은 메뉴의 **`문서 차례 (Markdown)`** 는 이 공간을 **글의 차례**로 내려받습니다. 발표를 만들 때 쓰는 그 순서 그대로이고, **AI는 관여하지 않습니다** — 순서는 `follows` 연결이거나 캔버스에 늘어놓은 배치이며 문장은 쓴 그대로입니다. 백업용 마크다운과 달리 id·좌표·연결 목록이 없어서, 바로 문서에 붙여 넣고 이어 쓸 수 있습니다.
...
- **내 AI 사용 내역**: 내 생각이 **언제, 무엇을 위해** AI 모델로 갔는지 직접 확인합니다. 7 / 30 / 90일 중에서 고릅니다.
  - Dream 생성, AI Assist, 기억에 질문, 살펴보고 답하기, Dream 발전, 덱 묶음 제목, 덱 부 나누기로 나뉩니다.
  - **실패한 호출도 나옵니다.** 실패해도 프롬프트는 이미 나갔기 때문입니다.
  - **비어 있다고 아무 일도 없었던 건 아닙니다.** 기록은 보존 기간이 지나면 삭제되므로, 화면이 보존 기간을 함께 알려 줍니다.
  - **검색·연결용 임베딩은 이 목록에 없습니다.** 여러 사람의 메모가 한 번에 묶여 나가서 한 사람 것으로 셀 수 없기 때문입니다. 대신 **지금 이 설치가 메모 본문을 밖으로 보내는지 아닌지**를 목록 위에 그대로 적어 둡니다.
