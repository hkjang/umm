...
네트워크가 끊긴 동안의 생성·수정은 브라우저 queue에 저장되고 상단 상태 표시에서 대기 수를 확인할 수 있습니다. 재연결하면 자동 동기화되며 같은 메모의 연속 수정은 마지막 상태로 합쳐집니다. 서버에 더 최신 버전이 있으면 자동 덮어쓰기하지 않고 비교 창을 엽니다. 내 버전, 서버 버전 또는 두 본문을 합친 버전을 선택해 새 버전으로 저장하세요.
