...
test "$(printf '%s' "$tools" | jq -r '[.result.tools[].name] | join(",")')" \
  = "capture_thought,connect_notes,create_note,find_contradictions,find_open_questions,get_connections,get_related_notes,list_clusters,list_dreams,list_lines,list_notes,list_presentations,list_spaces,make_presentation,preview_presentation,search_notes"
