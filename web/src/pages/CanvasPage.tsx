...
import { ActionIcon, Badge, Button, Group, Loader, Menu, Modal, Paper, Select, Stack, Text, TextInput, Tooltip } from '@mantine/core';
import { IconArrowRight, IconBrain, IconChevronDown, IconDots, IconDownload, IconFileTypePdf, IconFocusCentered, IconLayoutGrid, IconMarkdown, IconMoonStars, IconPhoto, IconPlus, IconSearch, IconShare, IconSparkles, IconTrash, IconX } from '@tabler/icons-react';
...
function CanvasInner() {
  ...
  useEffect(() => {
    ...
    setNodes(notes.map((note) => ({ id: note.id, type: 'postit', position: { x: note.x, y: note.y }, style: { width: note.width, height: note.height }, data: { note, dimmed: !!q && !`${note.title} ${note.content}`.toLocaleLowerCase().includes(q), relatedCount: note.relatedCount, onGather: discoverRelated, onPatch: persist, onDelete: remove,onRestore:restore } })));
  }, [notes, query, persist, remove, restore,discoverRelated]);
  ...
  return <div className="canvas-page">
    ...
    <Paper className="canvas-toolbar" radius="xl" p={7}><Group gap={6} wrap="nowrap"><Menu shadow="md"><Menu.Target><Button className="space-switcher" variant="light" color="yellow.8" size="compact-sm" rightSection={<IconChevronDown size={14}/>} style={{textTransform:'none'}}>{activeName}</Button></Menu.Target><Menu.Dropdown><Menu.Label>공간</Menu.Label>{spaces.map(space=><Menu.Item key={space.id} onClick={()=>navigate(`/space/${space.id}`)}>{space.name}</Menu.Item>)}<Menu.Divider/><Menu.Item leftSection={<IconPlus size={15}/>} onClick={()=>void addSpace()}>새 공간</Menu.Item></Menu.Dropdown></Menu><TextInput className="thought-search" aria-label="생각 검색" value={query} onChange={(e) => setQuery(e.currentTarget.value)} leftSection={<IconSearch size={17}/>} rightSection={query?<ActionIcon variant="subtle" onClick={() => setQuery('')} aria-label="검색어 지우기"><IconX size={15}/></ActionIcon>:null} placeholder="생각 검색" variant="filled" />
