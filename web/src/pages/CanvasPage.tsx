...
import { IconArrowRight, IconBrain, IconChevronDown, IconDownload, IconFileTypePdf, IconFocusCentered, IconLayoutGrid, IconMarkdown, IconMoonStars, IconPhoto, IconPlus, IconSearch, IconShare, IconSparkles, IconTrash, IconX } from '@tabler/icons-react';
...
function CanvasInner() {
  ...
  return <div className="canvas-page">
    ...
    <Paper className="canvas-toolbar glass" radius="xl" p={7}><Group gap={6} wrap="nowrap"><Menu shadow="md"><Menu.Target><Button variant="light" color="yellow.8" size="compact-sm" rightSection={<IconChevronDown size={14}/>} style={{textTransform:'none'}}>{activeName}</Button></Menu.Target><Menu.Dropdown><Menu.Label>Spaces</Menu.Label>{spaces.map(space=><Menu.Item key={space.id} onClick={()=>navigate(`/space/${space.id}`)}>{space.name}</Menu.Item>)}<Menu.Divider/><Menu.Item leftSection={<IconPlus size={15}/>} onClick={()=>void addSpace()}>새 공간</Menu.Item></Menu.Dropdown></Menu><TextInput value={query} onChange={(e) => setQuery(e.currentTarget.value)} leftSection={<IconSearch size={17}/>} rightSection={query?<ActionIcon variant="subtle" onClick={() => setQuery('')}><IconX size={15}/></ActionIcon>:null} placeholder="생각 검색" variant="unstyled" style={{flex:1}} /><Tooltip label="공간 공유"><ActionIcon variant="subtle" color="dark" onClick={()=>void openShare()} aria-label="공간 공유"><IconShare size={19}/></ActionIcon></Tooltip><Menu shadow="md"><Menu.Target><Tooltip label="내보내기"><ActionIcon variant="subtle" color="dark" aria-label="내보내기"><IconDownload size={19}/></ActionIcon></Tooltip></Menu.Target><Menu.Dropdown><Menu.Item leftSection={<IconMarkdown size={16}/>} onClick={()=>void downloadMarkdown()}>Markdown</Menu.Item><Menu.Item leftSection={<IconPhoto size={16}/>} onClick={()=>void downloadImage()}>Image (PNG)</Menu.Item><Menu.Item leftSection={<IconFileTypePdf size={16}/>} onClick={()=>void printPDF()}>PDF 인쇄</Menu.Item></Menu.Dropdown></Menu><Menu shadow="md"><Menu.Target><Tooltip label="선택한 생각 발전시키기"><ActionIcon loading={aiBusy} variant="subtle" color="grape" aria-label="AI 생각 도구"><IconBrain size={20}/></ActionIcon></Tooltip></Menu.Target><Menu.Dropdown><Menu.Label>AI는 조용히 도와줍니다</Menu.Label>{[['summarize','요약'],['questions','질문 만들기'],['expand','확장'],['challenge','반대 관점'],['actions','실행 항목']].map(([mode,label])=><Menu.Item key={mode} onClick={()=>void assist(mode)}>{label}</Menu.Item>)}</Menu.Dropdown></Menu><Tooltip label="관련 생각 군집 모으기"><ActionIcon variant="subtle" color="blue" onClick={()=>void clusterThoughts()} aria-label="생각 군집"><IconLayoutGrid size={20}/></ActionIcon></Tooltip><Tooltip label="직접 연결한 생각 모으기"><ActionIcon variant="subtle" color="grape" onClick={()=>gravity()} aria-label="Thought Gravity"><IconFocusCentered size={20}/></ActionIcon></Tooltip></Group></Paper>
