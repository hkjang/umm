...
- **공유·변경 재검증**: 멤버 목록·공간 내보내기뿐 아니라 댓글과 Dream도 현재 멤버십을 다시 확인합니다. 권한 회수 뒤에는 Dream의 Today 카드·이력·본문·출처를 숨기고 피드백·채택·재생성·발전을 거부하며, 댓글 해결·삭제는 terminal Problem Details로 종료해 오프라인 큐의 뒤 작업을 막지 않습니다.
- **검토 신호 보존**: “리뷰 요약에 활동 포함”을 꺼도 협업 활동만 숨기고 고정·기한·오래된 생각의 기본 검토 목록은 유지합니다.
- **관리 설정 업그레이드 호환성**: 구버전 관리자 화면이 새 남용 방지 필드를 생략해도 저장된 로그인·API·AI 한도를 최신 PostgreSQL 값에서 원자적으로 병합해 초기화하지 않습니다.
