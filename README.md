...
- **거짓 성공 없는 오프라인 저장**: 브라우저 저장 용량이 가득 찼거나 사이트 저장 권한이 차단되면 변경을 저장했다고 표시하지 않고 즉시 복구 안내를 보여 줍니다. 저장소 읽기 자체가 거부되어도 로그인 초기화와 오프라인 상태 표시는 계속 동작하며, 읽을 수 없는 기존 큐를 빈 큐로 덮어쓰지 않습니다.
