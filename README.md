...
현재 버전은 `v0.3.0`입니다. 버전은 로그인 화면과 프로필 컨텍스트 메뉴에 함께 표시됩니다.
...
```text
Docker image: umm:v0.3.0
Release file: umm-v0.3.0.tar.gz
```

연결망에서 GitHub Release의 `umm-vX.Y.Z.tar.gz` 한 파일을 반입한 뒤:

```bash
gzip -dc umm-v0.3.0.tar.gz | docker load
...
```bash
./scripts/release-image.sh 0.3.0
```

Git tag `v0.3.0`를 push하면 CI가 `VERSION` 일치 여부를 확인하고 `umm:v0.3.0`를 빌드한 뒤, Docker image만 담은 `umm-v0.3.0.tar.gz`를 GitHub Release에 첨부합니다.
