...
- **공유·변경 재검증**: 멤버 목록·공간 내보내기뿐 아니라 댓글 작성 transaction도 현재 멤버십을 끝까지 잠가, 권한 회수와 겹쳐도 이전 권한으로 본문·이벤트·webhook을 만들지 않습니다.
- **검토 신호 보존**: “리뷰 요약에 활동 포함”을 꺼도 협업 활동만 숨기고 고정·기한·오래된 생각의 기본 검토 목록은 유지합니다.
- **작은 pool·PWA 업그레이드 안전성**: 상한 1~2에서는 전용 LISTEN 대신 안전 폴링으로 모든 연결을 request에 보존하고 알림 목록은 한 연결만 순차 사용하며, 고정 manifest·service worker·아이콘은 매 릴리스 재검증합니다.
