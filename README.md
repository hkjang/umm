...
- **외부 임베딩까지 유지되는 AI 제외 경계**: 메모·공간과 현재 Gateway 설정을 외부 임베딩 응답 및 vector 저장이 끝날 때까지 잠급니다. 제외 설정이 먼저 확정되면 본문과 범위 검색어는 로컬에서만 처리하고, 임베딩 lease가 먼저 시작되면 설정 변경이 호출 종료까지 기다립니다.
